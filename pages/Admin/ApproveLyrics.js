...
const ApproveLyrics = () => {
  ...
  const fetchPending = async () => {
    setFetching(true);
    const res = await fetch('/api/admin/lyrics/pending');
  ...
  const handleConfirmAction = async () => {
    ...
    if (actionType === 'approve') {
      ...
      const res = await fetch('/api/admin/lyrics/approve', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...editedLyric, slug, id: selectedLyric.id }),
      });
      if (res.ok) {
        setPendingLyrics(prev => prev.filter(l => l.id !== selectedLyric.id));
        setSelectedLyric(null); setEditedLyric(null);
        showMsg('✓ Published to library', 'success');
      } else {
        showMsg('Error approving lyric', 'error');
      }
    } else {
      const res = await fetch('/api/admin/lyrics/reject', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: selectedLyric.id }),
      });
      if (res.ok) {
        setPendingLyrics(prev => prev.filter(l => l.id !== selectedLyric.id));
        setSelectedLyric(null); setEditedLyric(null);
        showMsg('✕ Submission discarded', 'error');
      } else {
        showMsg('Error rejecting lyric', 'error');
      }
    }
              ...
              <div className={styles.colLeft}>
                ...
                {editedLyric.thumbnail_url && (
                  <div className={styles.thumbRow}>
                    <img src={editedLyric.thumbnail_url} alt="Thumbnail" className={styles.thumbPreview} />
                  </div>
                )}
                ...
                <div className={styles.fieldsCard}>
                  ...
                  <Field label="Lyrics Writer" icon={<FaPenNib />}>
                    <input className={styles.input} type="text" value={editedLyric.lyrics_writer || ''} onChange={set('lyrics_writer')} />
                  </Field>
                  ...
                  <Field label="Slug (URL)" icon={<FaTag />}>
                    <div className={styles.slugRow}>
                      <input className={styles.input} type="text" value={editedLyric.slug || ''} onChange={set('slug')} placeholder="auto-generated on publish if empty" />
                      <button type="button" className={styles.slugGenBtn}
                        onClick={() => setEditedLyric(prev => ({ ...prev, slug: generateSlug(prev.title) }))}>
                        Generate
                      </button>
                    </div>
                  </Field>
                  ...
                  <Field label="Thumbnail URL" icon={<FaLink />}>
                    <input className={styles.input} type="text" value={editedLyric.thumbnail_url || ''} onChange={set('thumbnail_url')} />
                  </Field>
                  <Field label="Added By" icon={<FaUserEdit />}>
                    <input className={styles.input} type="text" value={editedLyric.added_by || ''} onChange={set('added_by')} />
                  </Field>
                </div>

                <div className={styles.fieldsCard}>
                  <h3 className={styles.cardTitle}>Description</h3>
                  <textarea className={styles.textarea} rows={4} value={editedLyric.description || ''} onChange={set('description')} placeholder="Short description about the track…" />
                </div>
              </div>
                ...
                <div className={styles.lyricsCard}>
                  <div className={styles.lyricsCardHeader}>
                    <div className={styles.lyricsCardTitle}>
                      ...
                      <span className={styles.charCount}>{(editedLyric.lyrics || '').length} chars</span>
                    </div>
                    <button type="button" className={styles.expandBtn} onClick={() => setLyricsExpanded(p => !p)}>
                  ...
                  <textarea
                    ...
                    value={editedLyric.lyrics || ''} onChange={set('lyrics')} placeholder="Original lyrics…" />
                </div>
                ...
                <div className={styles.lyricsCard}>
                  <div className={styles.lyricsCardHeader}>
                    <div className={styles.lyricsCardTitle}>
                      ...
                      <span className={styles.charCount}>{(editedLyric.english_lyrics || '').length} chars</span>
                    </div>
                    <button type="button" className={styles.expandBtn} onClick={() => setEnExpanded(p => !p)}>
                  ...
                  <textarea
                    ...
                    value={editedLyric.english_lyrics || ''} onChange={set('english_lyrics')} placeholder="English translation (optional)…" />
                </div>

                <div className={styles.infoStrip}>
                  <InfoChip label="ID" value={selectedLyric.id} />
                  <InfoChip label="Created" value={selectedLyric.created_at ? new Date(selectedLyric.created_at).toLocaleDateString() : '—'} />
                  <InfoChip label="Source" value={selectedLyric.added_by?.includes('(public)') ? 'Public submission' : 'Admin'} />
                  {selectedLyric.submitter_email && <InfoChip label="Email" value={selectedLyric.submitter_email} />}
                </div>
              </div>
            </div>
          </main>
